export default function HeroBannerComponent ({ products }) {
  return (
    <div className='flex justify-center mt-10'>
      <div className='py-[100px] px-[40px] bg-[#dcdcdc] md:rounded-2xl relative h-[500px] leading-[0.9] w-screen md:w-[90%]'>
        <p className='text-xl'>{products[0].smallText}</p>
        <h3 className='text-[4rem] mt-1'>{products[0].midText}</h3>
        <h1 className='text-white text-[10rem] ml-[-20px] uppercase'>{products[0].largeText}</h1>
        <img className='absolute top-0 right-[20%] w-[450px] h-[450px]' src={products[0].img} alt='headphones' />
        <div>
          ...
          <div className='absolute right-[10%] bottom-[5%] w-[300px] leading-[1.3] flex flex-col text-[#324d67]'>
            ...
            <p className='text-[#5f5f5f] font-thin text-end'>{products[0].description}</p>
          </div>
        </div>
      </div>
    </div>
