...
export default function CartComponet ({ cartItems, setShowCart, totalPrice }) {
  return (
    <div className='cart-wrapper w-screen bg-[#00000080] h-screen fixed right-0 top-0 bottom-0 z-[100] transition-all duration-1000 ease-in-out'>
      <div className='cart-container p-1 h-screen w-screen  md:w-[650px] dark:bg-gray-700 bg-white float-right relative py-0 px-[10px]'>
        <button
          type='button'
            ...
            <div className='product mb-6 flex gap-[30px] sm:p-5' key={item.Product_ID}>
              ...
              <div className='item-desc'>
                <div className='flex justify-between w-52 sm:w-[350px] text-[#324d67] top'>
                  <h5 className='font-medium text-lg sm:text-2xl dark:text-gray-300'>{item.Product_Name}</h5>
                  <h4 className='text-xl dark:text-gray-100 text-black'>{(item.price).toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'USD'
                  })}
                  </h4>
